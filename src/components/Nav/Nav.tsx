...
const Nav = () => {
    ...
    const textStyling = { margin: customSpacing.none, marginLeft: customSpacing.m, color: theme.palette.text.primary };
    ...
    return (
        <div id="Nav">
            <button onClick={ () => toggleDrawer(true) }><MenuIcon style={ iconStyling } /></button>
            <Drawer className="drawers" open={ open } onClose={ () => toggleDrawer(false) }>
                <Box className="nav__icon" sx={{ marginBottom: customSpacing.l }}>
                ...
                <Link to="/search" className={ `nav__link ${ pathname === '/search' ? 'active': '' }` }
                    style={{ marginBottom: customSpacing.m }}
                    onClick={ () => toggleDrawer(false) }>
                    <SearchOutlinedIcon style={ iconStyling } />
                    <p style={ textStyling }>Search</p>
                </Link>
                <Link to="/my-stocks" className={ `nav__link ${ pathname === '/my-stocks' ? 'active': '' }` }
                    style={{ marginBottom: customSpacing.m }}
                    onClick={ () => toggleDrawer(false) }>
                    <AttachMoneyOutlinedIcon style={ iconStyling } />
                    <p style={ textStyling }>My Stocks</p>
                </Link>
                <Link to="/news" className={ `nav__link ${ pathname === '/news' ? 'active': '' }` }
                    style={{ marginBottom: customSpacing.m }}
                    onClick={ () => toggleDrawer(false) }>
                    <FeedOutlinedIcon style={ iconStyling } />
                    <p style={ textStyling }>News & Media</p>
                </Link>
                <Link to="/settings" className={ `nav__link ${ pathname === '/settings' ? 'active': '' }` }
                    style={{ marginBottom: customSpacing.m }}
                    onClick={ () => toggleDrawer(false) }>
