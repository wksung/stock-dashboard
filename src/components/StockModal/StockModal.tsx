...
const StockModal = ({
    ...
    const addStock = () => {
        ...
        if (findMyStocks === null) {
            const myStocks = [{ stock, price, quantity }];
            localStorage.setItem('myStocks', JSON.stringify(myStocks));
        } else {
            const parsedStocks = JSON.parse(findMyStocks);
            let updatedStocks: any[];

            const temp = parsedStocks.find(
                (s: { stock: string }) => s.stock === stock,
            );
            if (temp !== undefined && Object.keys(temp).length) {
                temp.price = price;
                temp.quantity = quantity;
                const updatedArray = parsedStocks.map((item: { stock: any }) =>
                    item.stock === temp.stock ? temp : item,
                );
        ...
        <div>
            ...
            >
                <div
                    ...
                    style={{
                        paddingTop: customSpacing.l,
                        paddingLeft: customSpacing.lxl,
                        paddingBottom: customSpacing.l,
                        paddingRight: customSpacing.lxl,
                    }}
                >
                    <p
                        className="modal-container--title"
                        style={{ color: theme.palette.text.primary }}
                    >
                        {type === 'add' ? 'Add to my stocks' : `Edit ${stock}`}
                    </p>
                    <TextField
                        inputProps={{ 'data-testid': 'popup__modal-ticker' }}
                        style={{
                    ...
                    />
                    <TextField
                        inputProps={{ 'data-testid': 'popup__modal-price' }}
                        style={{
                            fontSize: theme.typography.body2.fontSize,
                            marginTop: customSpacing.s,
                        }}
                    ...
                    >
                        Current Price: {money(amount)}
                    </small>
                    <TextField
                        inputProps={{ 'data-testid': 'popup__modal-quantity' }}
                        style={{
                            fontSize: theme.typography.body2.fontSize,
                            marginTop: customSpacing.s,
                        }}
