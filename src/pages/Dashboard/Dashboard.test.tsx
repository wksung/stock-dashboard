...
const MockGeneralNews = [
    {
        ...
        id: 7418174,
        url: 'https://www.marketwatch.com/story/united-airlines-sheds-unprofitable-capacity-in-better-than-expected-quarter-00c76d2b',
    },
    {
        headline:
            'WNBA players only get 9.3% of league revenue — here’s how much NBA, NFL and NHL players get',
        id: 7418175,
        url: 'https://www.marketwatch.com/story/wnba-players-only-get-9-3-of-league-revenue-heres-how-much-nba-nfl-and-nhl-players-get-0abef80c',
    },
];
...
const ProfilesTickerMap: Record<
    'TSLA' | 'NVDA',
    (typeof MockCompanyProfiles)[number]
> = {
...
describe('Dashboard', () => {
    ...
    beforeEach(() => {
        ...
        (getCompanyProfile as jest.Mock).mockImplementation(
            (ticker: 'TSLA' | 'NVDA') =>
                ProfilesTickerMap[ticker]
                    ? Promise.resolve(ProfilesTickerMap[ticker])
                    : Promise.resolve([]),
        );
        (getCompanyRecommendations as jest.Mock).mockImplementation(
            (ticker: 'TSLA' | 'NVDA') =>
                RecommendationsTickerMap[ticker]
                    ? Promise.resolve(RecommendationsTickerMap[ticker])
                    : Promise.resolve([]),
        );
    ...
    describe('Market Status', () => {
        it('should show the market status as being closed', async () => {
            await act(async () => render(<MockDashboard />));
            expect(
                screen.getByText(/MARKET STATUS: CLOSED/i),
            ).toBeInTheDocument();
        });
        ...
        it('should show the market status as being open', async () => {
            ...
            await act(async () => render(<MockDashboard />));
            expect(
                screen.getByText(/MARKET STATUS: OPEN/i),
            ).toBeInTheDocument();
        });
        ...
        it('should show the correct time of the market', async () => {
            ...
            const currentTime = new Date(
                MockMarketStatus.t * 1000,
            ).toLocaleString('en-US', { timeZone: MockMarketStatus.timezone });
            expect(
                screen.getByText(new RegExp(currentTime)),
            ).toBeInTheDocument();
        });
    });
    ...
    describe('Portfolio Score', () => {
        it('should show your score', async () => {
            ...
            const portfolioScore =
                await screen.findByTestId('portfolio__score');
            expect(
                portfolioScore != null &&
                    portfolioScore.textContent != null &&
        ...
        it('should show the information popup', async () => {
            await act(async () => render(<MockDashboard />));
            const portfolioPopupBtn = await screen.findByTestId(
                'portfolio__score-btn',
            );

            await act(async () => portfolioPopupBtn.click());
            const portfolioPopup = await screen.findByTestId(
                'portfolio__score-btn-modal',
            );
    ...
    describe('Portfolio Value', () => {
        it('should show the total portfolio value', async () => {
            ...
            const portfolioValue =
                await screen.findByTestId('portfolio__value');
            expect(portfolioValue.textContent).not.toEqual('$0.00');
        });

        it('should show the profit amount in dollars', async () => {
            await act(async () => render(<MockDashboard />));
            const portfolioValueDollar = await screen.findByTestId(
                'portfolio__profit_in_d',
            );
        ...
        it('should show the profit amount in percentages', async () => {
            await act(async () => render(<MockDashboard />));
            const portfolioValuePercentage = await screen.findByTestId(
                'portfolio__value_in_p',
            );
    ...
    describe('Portfolio Industries', () => {
        it('should show all the industries', async () => {
            ...
            const portfolioIndustry =
                await screen.findAllByTestId(/portfolio__industry-/i);
