...
const MockCompanyQuotes = [
    { "c": 221.33, "d": 1.76, "dp": 0.8016, "h": 222.8199, "l": 218.93, "o": 221.4, "pc": 219.57, "t": 1729108800 },
    { "c": 135.72, "d": 4.12, "dp": 3.1307, "h": 136.62, "l": 131.58, "o": 133.98, "pc": 131.6, "t": 1729108800 }
]
...
const CompanyTickerMap: Record<'TSLA' | 'NVDA', typeof MockCompanyProfile[number]> = {
...
describe("MyStocks", () => {
    ...
    beforeEach(() => {
        ...
        (getCompanyProfile as jest.Mock).mockImplementation((ticker: 'TSLA' | 'NVDA') => CompanyTickerMap[ticker] ? Promise.resolve(CompanyTickerMap[ticker]) : Promise.resolve([]));
    });
    ...
    it("should show messaging if you have no stocks available", async () => {
        ...
        const noStocksMessaging = screen.getByText(/You currently have no stocks in your list./i);
    ...
    describe("Table View", () => {
        ...
        it("should show the current price of your stocks", async () => {
            await act(async () => render(<MockMyStocksDiv />));
            const totalCurrentPrice = screen.getByTestId("total__current__price");
    ...
    describe("Update & Delete Stocks", () => {
        ...
        it("should be able to update a specific stock in your list", async () => {
            ...
            await act(async () => fireEvent.change(popUpModalPrice, { target: { value: "200" } }));
