...
const MockCompanyNews = [
    {
        "category": "company",
        "datetime": 1729073010,
        "headline": "Tesla receives approval for first stage of German plant expansion",
        "id": 130482460,
        "image": "https://www.just-auto.com/wp-content/uploads/sites/30/2024/10/tesla-shutterstock_2500278999.jpg",
        "related": "TSLA",
        "source": "Yahoo",
        "summary": "The three-stage expansion aims to double the plant’s production capacity from 500,000 to one million vehicles per year.",
...
const MockCompanyQuotes = { "c": 219.57, "d": 0.41, "dp": 0.1871, "h": 224.26, "l": 217.12, "o": 220.01, "pc": 219.16, "t": 1729022400 };
...
describe("Search", () => {
    ...
    beforeEach(() => {
        ...
        (getCompanyRecommendations as jest.Mock).mockResolvedValue(MockCompanyRecommendations);
    });
    ...
    describe("Search Result Output", () => {
        it("should show up an error message when you search nothing", async () => {
            ...
            (getCompanyQuote as jest.Mock).mockResolvedValue({"c":0,"d":null,"dp":null,"h":0,"l":0,"o":0,"pc":0,"t":0});
            (getCompanyProfile as jest.Mock).mockResolvedValue({});
            (getCompanyRecommendations as jest.Mock).mockResolvedValue([]);
    
            await act(async () => render(<MockSearch />));
            const inputField = screen.getByTestId("search__input");
            await act(async () => fireEvent.change(inputField, { target: { value: "" } }));
        ...
        it("should show you information of the stock", async () => {
            await act(async () => render(<MockSearch />));
            const inputField = screen.getByTestId("search__input");
            await act(async () => fireEvent.change(inputField, { target: { value: "TSLA" } }));
            const searchStockBtn = screen.getByTestId("search__btn");
            await act(async () => fireEvent.click(searchStockBtn));

            expect(screen.getByTestId("output__ticker").textContent).toBe(`Ticker: ${ MockCompanyProfile.ticker }`);
        });
        ...
        it("should show you information of the current price of the stock", async () => {
            ...
            expect(screen.getByTestId("output__current-price").textContent).toBe(`Current Price: $${ MockCompanyQuotes.c }`);
        });
        ...
        it("should show you recommendation of the stocks", async () => {
            ...
            const getRecommendedTrends = await screen.findAllByTestId(/trends-/i);
            expect(getRecommendedTrends.length).toBe(MockCompanyRecommendations.length);
        });
        ...
        it("should show you the news of the stock", async () => {
            ...
            const getAllNewsArticle = await screen.findAllByTestId(/news__item-/i);
    ...
    describe("Add to my stocks", () => {
        ...
        it("should add to my stocks when you add all the info and press the popup button", async () => {
            ...
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-ticker"), { target: { value: "TSLA" } }));
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-price"), { target: { value: "100" } }));
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-quantity"), { target: { value: "50" } }));
        ...
        it("should show an error when you add nothing in the info and press the popup button", async () => {
            ...
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-ticker"), { target: { value: "TSLA" } }));
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-price"), { target: { value: "0" } }));
            await act(async () => fireEvent.change(screen.getByTestId("popup__modal-quantity"), { target: { value: "0" } }));
            const popUpBtn = screen.getByText(/Add stock/i);
            await act(async () => fireEvent.click(popUpBtn));
            const errorMessage = await screen.findByText(/Please add your price or quantity./i);
