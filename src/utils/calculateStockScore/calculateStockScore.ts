...
export const calculateStockScore = (stockRatings: CompanyRecommendations[][], stockData: MyStock[]): number => {
    const ratingWeights = {
      strongBuy: 1.5,
      buy: 1.0,
      hold: 0.5,
    ...
    stockRatings.forEach((ratingsGroup: CompanyRecommendations[]) => {
      ...
      const totalRatingsCount = latestRating.strongBuy + latestRating.buy + latestRating.hold + latestRating.sell + latestRating.strongSell;
